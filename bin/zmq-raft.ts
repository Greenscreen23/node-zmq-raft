...
if (require.main !== module)
    throw new Error('zmq-raft.js must be run directly from node');

import path from 'path';
import { parse as parseUrl } from 'url';
import program from 'commander';
import debugFactory from 'debug';
const debug = debugFactory('zmq-raft');
...
readConfig(opts.config, opts.ns)
    .then((config) => {
        ...
        return raft.server.builder.build(config).then((zmqRaft) => {
            ...
            console.log(
                ...
                zmqRaft.url || zmqRaft.routerBindUrl
            );
        });
    })
    .catch((err) => {
