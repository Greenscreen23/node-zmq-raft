...
const min = Math.min;
export const MAX_ALLOWED_INTEGER = Number.MAX_SAFE_INTEGER || 9007199254740991;
export const MIN_ALLOWED_INTEGER = Number.MIN_SAFE_INTEGER || -9007199254740991;
...
export function bufferToBool(buffer) {
  return (buffer.length !== 0 && !!buffer[0]);
}

export function allocBufWriter(value, writer) {
 ...
 * @return {Buffer}
**/
export function allocBufUIntLE(value, typecheck?) {
 ...
 * @return {Buffer}
**/
export function allocBufIntLE(value, typecheck) {
 ...
 * @return {Buffer}
**/
export function allocBufNumberLE(value, typecheck) {
 ...
 * @return {number}
**/
export function writeBufUIntLE(value, buffer, offset?, stop?) {
  value = +value;
  offset = offset >>> 0;
 ...
 * @return {number}
**/
export function writeBufIntLE(value, buffer, offset?, stop?) {
  value = +value;
  offset = offset >>> 0;
  if (value > MAX_ALLOWED_INTEGER) throw new Error("value is above maximum allowed integer");
 ...
 * @return {number}
**/
export function writeBufNumberLE(value, buffer, offset?) {
  value = +value;
 ...
 * @return {number|null}
**/
export function readBufUIntLE(buffer, offset?, stop?) {
  offset = offset >>> 0;
  var limit = buffer.length - 1;
  if (stop !== undefined && limit > (stop = (stop >> 0) - 1)) limit = stop;
 ...
 * @return {number|null}
**/
export function readBufIntLE(buffer, offset, stop) {
  offset = offset >>> 0;
  var limit = buffer.length - 1;
  if (stop !== undefined && limit > (stop = (stop >> 0) - 1)) limit = stop;
  if (offset > limit) return null;
 ...
 * @return {number|null}
**/
export function readBufNumberLE(buffer, offset, stop) {
  offset = offset >>> 0;
...
b.fill(0);
const MAX_ALLOWED_INTEGER = require('./lib/utils/bufconv').MAX_ALLOWED_INTEGER;
const MIN_ALLOWED_INTEGER = require('./lib/utils/bufconv').MIN_ALLOWED_INTEGER;
const MAX_48=(2**48)-1;
var {allocBufUIntLE} = require('./lib/utils/bufconv');
var {allocBufIntLE} = require('./lib/utils/bufconv');
var {allocBufNumberLE} = require('./lib/utils/bufconv');
var {writeBufUIntLE} = require('./lib/utils/bufconv');
var {readBufUIntLE}  = require('./lib/utils/bufconv');
var {writeBufIntLE}  = require('./lib/utils/bufconv');
var {readBufIntLE}  = require('./lib/utils/bufconv');
var {readBufNumberLE}  = require('./lib/utils/bufconv');
...
ben(10000000,()=>{writeBufUIntLE(MAX_48,b,0,6)})
ben(10000000,()=>{b.writeUIntLE(MAX_48,0,6)})
ben(10000000,()=>{readBufUIntLE(b,0,6)})
ben(10000000,()=>{b.readUIntLE(0,6)})
*/
