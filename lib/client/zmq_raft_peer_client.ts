...
import { assertConstantsDefined, parsePeers } from '../utils/helpers';
import { bufferToLogEntry } from '../common/log_entry';
import { bufferToSnapshotChunk } from '../common/snapshot_chunk';
...
class RequestEntriesStream extends Readable {
  ...
  _read(_size) {
    ...
    if (request != null) {
      request().catch(err => this.destroy(err));
    }
  }
}
...
export default exports = ZmqRaftPeerClient;
